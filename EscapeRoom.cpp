    ...
    */
    ...
    {
        if (wantFinal) return FINAL_CLUE_INDEX;
        ...
        if (cnt == 0) {
            for (int i = 0; i < FINAL_CLUE_INDEX; i++)
                if (!USED_CLUES[i]) valid[cnt++] = i;
        }

        if (cnt == 0) return rand() % FINAL_CLUE_INDEX;

        return valid[rand() % cnt];
    }
    Clue pickRandomClueForRoom(const string& roomType,
                            const string& roomDifficulty,
                            bool wantFinal = false)
    {
        if (wantFinal) {
    ...
    void printRoom(const Room* r, int score) {
        ...
        if (r->roomType == "EXIT") {
            ...
            cout << "  1) Door 1 -> ";
            if (r->next1) cout << "Room " << r->next1->roomID << "\n";
            else cout << "[NONE]\n";
        }
        else {
            cout << "  1) Door 1 -> "
    ...
    void addToAll(GameMap &gm, Room* r) {
        gm.all[gm.count++] = r;
    }
    ...
    GameMap buildMap() {
        ...
        addToAll(gm, EN1); addToAll(gm, EN2); addToAll(gm, EN3); addToAll(gm, EN4);
        addToAll(gm, I1); addToAll(gm, I2); addToAll(gm, I3); addToAll(gm, I4);
        addToAll(gm, I5); addToAll(gm, I6); addToAll(gm, I7); addToAll(gm, I8);
        addToAll(gm, EX1); addToAll(gm, EX2);

        // Base connections
        EN1->next1 = I1; I1->prev = EN1;
        I1->next1 = I2; I2->prev = I1;

        EN4->next1 = I3; I3->prev = EN4;
        I3->next1 = I4; I4->prev = I3;

        EN2->next1 = I5; I5->prev = EN2;
        I5->next1 = I6; I6->prev = I5;
        I6->next1 = EX2; // prev will be set dynamically when moving

        EN3->next1 = I7; I7->prev = EN3;
        I7->next1 = I8; I8->prev = I7;
        I8->next1 = I6; // merge
        ...
        Room* roomsToAssign[] = {EN1, EN2, EN3, EN4, I1, I2, I3, I4, I5, I6, I7, I8, EX1, EX2};
        int total = 14;
        ...
        r->clues[0].usedHint = false; r->clues[0].attempts = DEFAULT_ATTEMPTS;
        r->clues[1].usedHint = false; r->clues[1].attempts = DEFAULT_ATTEMPTS;
    ...
    int main() {
        ...
        while (true) {
            ...
            if (!ok) {
                cout << "You stay in the same room.\n";
                continue;
            }
